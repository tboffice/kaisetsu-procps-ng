...
　やってまいりました、procps-ngの花形 `top` コマンドです。manコマンドを打つと1500行くらいあるんですねー渋いですねー。
ガッツリ解説していきます。まずはDESCRIPTIONを読んでいきましょう。雑に訳したので、ちゃんとした文章は原文を見てね。
...
　topの解説には大きく分けて、コマンドラインオプション、サマリー、フィールド・コラム表示、インタラクティブコマンド、ウインドウを行ったきり来たり、ファイルの仕様、裏技があります。
...
-o fieldname
   fieldnameでソートする。実行例は `top -o +PID` とか `top -o -PID` とか。オーダーを指定する

-O
   fieldnameを表示して終わる。上のコマンドの filedname を調べるときに使う
