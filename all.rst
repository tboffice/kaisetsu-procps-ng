...
-o fieldname
   fieldnameでソート。実行例は `top -o +PID` とか `top -o -PID` とか。オーダーを指定

-O
   fieldnameを表示します。それだけ。上のコマンドの filedname を調べるときに使う

-p
     PIDを指定して表示。たとえばこんな感じ。 `top -p1 -p2 -p3` または `top -p1,2,3` 。PIDは、20個まで指定できます。PIDには、0も指定できます。何が起こるかやってみましょう。解除するには、Uかuを打ってenterを押す

-s
   設定ファイルを読み込まないセキュアモードで起動 [#securemode]_ 。topに設定ファイルってあるんですねーあとで解説します

.. [#securemode] セキュアラモードってかわいくないですか（なに言ってんだこいつ
...
便宜上a,b,c,dという記号を付けました。a は us + ni を足したパーセンテージ。bは sy のパーセンテージ。cは合計。dはグラフです。us ってなんだよ、って感じですが、propsのときはこんな感じで表示されます。
...
えーっとこの辺を説明しているとカーネルの話になってくるので、詳しく知りたい方はLinuxカーネルの本やwebをあたって下さい。
次はメモリ使用量です。この辺も深みにはまるのでサクッと解説だけにとどめておきます。たとえばこんな感じになっているとします。
...
便宜上a,b,c と記号を振りました。aは使われているパーセンテージです。bはトータルのメモリ量、cはグラフです。気をつけて欲しいのは、 procps の時代は容量で表されていたところがパーセンテージになっています。注意が必要です。
さらに詳細な情報は、mでトグルすることが可能で４種類のモードがあります。実際にmを押してみよう！
...
です。もうお分かりですね。フィールドは実は58個もあります。fキーを押すと、どのフィールドを表示するか決める画面に移ります。全部説明すると大変なことになるので、操作の方法だけ。
fキーを押すと「Fields Management for window」と出てくるのであとは、そこにある解説の通りに操作します。dキーまたは  <space> キーで表示・非表示の選択、sでソート順の決定、qか <Esc>で完了です。aかwキーが隠しコマンドです。押してみて自分で確かめよう！
